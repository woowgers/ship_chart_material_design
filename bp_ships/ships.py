...
def index():
    ...
    try:
        result = make_query(
            db_config,
            'SELECT s.ship_id, s.ship_name, p.port_name, s.img_url, p.id FROM ships s INNER JOIN ports p ON s.port_id = p.id',
            [],
        )
    except ValueError:
        print('ValueError')
    ...
    return render_template(
        'index_ships.html', title='Ships', page='ships', ships=result,
    )
...
def ship():
    ...
    if request.method == 'POST':
        ...
        try:
            make_query(
                db_config,
                'INSERT INTO ships VALUES(NULL, %s, %s, %s)',
                [request.form['ship-name'], request.form['port-id'], filename],
            )
        except ValueError:
            print('ValueError')
    ...
    if request.method == 'PUT':
        ...
        if file:
            try:
                make_query(
                    db_config,
                    'UPDATE ships SET ship_name=%s, port_id=%s, img_url=%s WHERE ship_id=%s',
                    [
            ...
            try:
                make_query(
                    db_config,
                    'UPDATE ships SET ship_name=%s, port_id=%s WHERE ship_id=%s',
                    [
                        request.form['ship-name'],
                        request.form['port-id'],
                        request.form['ship-id'],
