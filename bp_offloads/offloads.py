...
def offload():
    ...
    if request.method == 'GET':
        ...
        try:
            offload_result = make_query(
                ...
                [request.args['offload_id']],
            )[0]
            emp_off_result = make_query(
                db_config,
            ...
            offload_data = {
                'offload_id': offload_result[0],
                'entry_dt': dt.render(dt=offload_result[1]),
                'exit_dt': dt.render(dt=offload_result[2]),
                'ship_id': offload_result[3],
                'ship_name': offload_result[4],
                'port_id': offload_result[5],
                'port_name': offload_result[6],
                'img_url': offload_result[7],
                'employee_offloads': [],
            }
                ...
                offload_data['employee_offloads'].append(
                    {
                        ...
                        'admission': dt.render(dt=row[5]),
                        'dismission': dt.render(dt=row[6]),
                        'department_id': row[7],
                        'img_url': row[8],
                        'offload_date': dt.render(dt=row[9]),
                        'work_hours': row[10],
                    },
                )
    ...
    if request.method == 'POST':
        ...
        try:
            make_query(
                db_config,
                'INSERT INTO offloads VALUES(NULL, %s, NULL, %s)',
                [content['entry_dt'], content['ship_id']],
            )
            offload_id = make_query(
                db_config, 'SELECT MAX(offload_id) FROM offloads', [],
            )[0][0]
            for oe in content['offload_employees']:
                make_query(
                    db_config,
                    'INSERT INTO emp_off VALUES(NULL, %s, %s, %s, %s, 0)',
                    [
                        oe['employee_id'],
                        oe['offload_date'],
                        oe['work_hours'],
    ...
    if request.method == 'PUT':
        ...
        try:
            ...
            make_query(
                db_config,
                'UPDATE offloads SET entry_dt=%s, ship_id=%s WHERE offload_id=%s',
                [content['entry_dt'], content['ship_id'], offload_id],
            )
            make_query(
                db_config,
                'DELETE FROM emp_off WHERE offload_id=%s',
                [offload_id],
            )
            for oe in content['offload_employees']:
                make_query(
    ...
    if request.method == 'DELETE':
        ...
        try:
            for offload_id in content['offloads_id']:
                make_query(
                    db_config,
                    'DELETE FROM offloads WHERE offload_id = %s',
                    [offload_id],
                )
        except ValueError:
            print('ValueError')
...
def finish():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            make_query(
                db_config,
                'UPDATE offloads SET exit_dt=%s WHERE offload_id=%s',
    ...
    if request.method == 'POST':
        ...
        try:
            for offload_id in content['offloads_id']:
                ...
                make_query(
                    db_config,
                    'UPDATE emp_off SET history=1 WHERE offload_id=%s',
                    [offload_id],
                )
        except ValueError:
            print('ValueError')
